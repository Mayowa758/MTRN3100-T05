...
const char commandString[] = "fflflfrfrffrfff";
...
// Side LiDARs are an emergency correction only. They do not perform continuous
// wall following and do not affect the robot above the activation distance.
const int SIDE_EMERGENCY_ENTER_MM = 35;
const int SIDE_EMERGENCY_EXIT_MM = 50;
const int SIDE_EMERGENCY_CORRECTION = 10;
const int SIDE_EMERGENCY_CONFIRMATIONS = 2;
const int FRONT_STOP_MM = 10;
...
enum SideCorrectionState {
    SIDE_CLEAR,
    TOO_CLOSE_LEFT,
    TOO_CLOSE_RIGHT
};

SideCorrectionState sideCorrectionState = SIDE_CLEAR;
...
int calculateLidarCorrection(int leftMM, int rightMM) {
    ...
    // Require repeated close readings so one noisy sample cannot turn the car.
    const bool leftClose = leftValid && leftMM <= SIDE_EMERGENCY_ENTER_MM;
    const bool rightClose = rightValid && rightMM <= SIDE_EMERGENCY_ENTER_MM;
    ...
    // Hysteresis: once correction starts, keep it until that wall is beyond
    // the larger EXIT distance. If the opposite wall becomes even closer,
    // change direction so the robot always moves toward the greater clearance.
    if (sideCorrectionState == TOO_CLOSE_LEFT) {
        if (rightConfirmed && (!leftValid || rightMM < leftMM)) {
            sideCorrectionState = TOO_CLOSE_RIGHT;
        } else if (!leftValid || leftMM >= SIDE_EMERGENCY_EXIT_MM) {
            sideCorrectionState = SIDE_CLEAR;
            leftCloseCount = 0;
        }
    } else if (sideCorrectionState == TOO_CLOSE_RIGHT) {
        if (leftConfirmed && (!rightValid || leftMM < rightMM)) {
            sideCorrectionState = TOO_CLOSE_LEFT;
        } else if (!rightValid || rightMM >= SIDE_EMERGENCY_EXIT_MM) {
            sideCorrectionState = SIDE_CLEAR;
            rightCloseCount = 0;
        }
    }

    if (sideCorrectionState == SIDE_CLEAR && leftConfirmed && rightConfirmed) {
        // Both sides are close: move toward whichever side has more clearance.
        if (leftMM > rightMM) sideCorrectionState = TOO_CLOSE_RIGHT;
        else if (rightMM > leftMM) sideCorrectionState = TOO_CLOSE_LEFT;
    } else if (sideCorrectionState == SIDE_CLEAR && leftConfirmed) {
        sideCorrectionState = TOO_CLOSE_LEFT;
    } else if (sideCorrectionState == SIDE_CLEAR && rightConfirmed) {
        sideCorrectionState = TOO_CLOSE_RIGHT;
    }
    ...
    if (sideCorrectionState == TOO_CLOSE_RIGHT) {
        return SIDE_EMERGENCY_CORRECTION;
    }
...
bool driveForwardOneCell() {
    ...
    // Do not carry an emergency correction over from a previous cell or turn.
    sideCorrectionState = SIDE_CLEAR;
    leftCloseCount = 0;
    rightCloseCount = 0;
