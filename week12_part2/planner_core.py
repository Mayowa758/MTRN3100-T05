...
def plan_course(
    ...
    goal_direction = (
        goal_outside[0] - goal_grid[0],
        goal_outside[1] - goal_grid[1],
    )
    grid_path = grid_astar(
        occupancy,
        start_grid,
        goal_grid,
        config,
        turn_occupancy=turn_occupancy,
        initial_heading_deg=initial_heading,
        goal_direction=goal_direction,
    )
    full_grid_path = [start_outside, *grid_path, goal_outside]
    path_mm = [grid_cell_center(cell, config) for cell in full_grid_path]
    motions = path_to_motions(path_mm, initial_heading)
    commands = grid_path_to_commands(full_grid_path, initial_heading)
    return PlanResult(
        rectified, dark_mask, occupancy, obstacles, path_mm, motions, commands, homography
    )
...
def save_arduino_header(path: str | Path, result: PlanResult, pre_commands: str, post_commands: str) -> None:
    ...
    lines = [
        ...
        f'const char COURSE_COMMANDS[] = "{result.grid_commands}";',
