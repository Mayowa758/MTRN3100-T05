...
def show_full_result(
    ...
    messagebox.showinfo(
        ...
        f"Obstacle course: {course_commands}\n"
        f"After course: {post_commands}\n\n"
        f"Complete commands: {pre_commands}{course_commands}{post_commands}",
        parent=root,
    )
    root.destroy()
...
def main() -> int:
    ...
    save_route_json(
        ...
        extra={
            ...
            "complete_commands": pre_commands + course_result.grid_commands + post_commands,
        },
    )
    save_arduino_header(
